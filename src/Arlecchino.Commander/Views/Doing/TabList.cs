...
///
/// The band takes a click on a tab, which is enough while there are three of them and no use at all over ssh
/// from a terminal that reports no mouse. This is the other way in. <c>F2</c> shows the tabs with the two
/// things worth doing to them, and the palette behind <c>Ctrl+K</c> shows the same tabs among everything
/// else, so the tab on a server is reached by typing the name of the server.
/// </summary>
public static class TabList
{
    ...
    /// <summary>
    /// One row per tab there is, named the way its own tab in the band is named so the row and the thing it
    /// points at read alike. The number leads the name rather than sitting beside it because a list narrows
    /// on what is written in the row. Two tabs on the same pair of folders are told apart by typing the
    /// number, and the tab on a server by typing the server.
    ///
