...
///
/// Three shapes of this call catch people out. The path list is double-null-terminated — one terminator
...
///
/// The third is the struct itself, and it is the one that crashes rather than lies: the header packs it
/// to a byte only when the process is 32-bit, so a 64-bit process must lay the same fields out at their
/// natural alignment. Get that wrong and the shell reads the path pointer out of the middle of two other
/// fields and walks off into memory that is not a path. Hence two of them, one per bitness.
/// </summary>
